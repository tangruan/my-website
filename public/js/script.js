const AudioApp = (function () {
  ...
  function init() {
    ...
    // 添加以下代码来实现文本同步
    const aiTextInput = document.getElementById("text-input");
    const recordTextInput = document.getElementById("record-text-input");

    aiTextInput.addEventListener("input", function() {
      recordTextInput.value = this.value;
    });
    ...
    audioWorker.onmessage = function(e) {
      if (e.data.type === 'audioProcessed') {
        handleAudioProcessResult(e.data.result);
      }
    };
  }
  ...
  function bindEventListeners() {
    document.querySelector(
      'button[onclick="AudioApp.generateAIVoice()"]'
    ).onclick = generateAIVoice;
    document.querySelector(
      'button[onclick="AudioApp.startRecording()"]'
    ).onclick = startRecording;
    document.querySelector(
      'button[onclick="AudioApp.stopRecording()"]'
    ).onclick = stopRecording;
    document.querySelector(
      'button[onclick="AudioApp.uploadRecording()"]'
    ).onclick = uploadRecording;
    document.querySelector(
      'button[onclick="AudioApp.retryRecording()"]'
  ...
  function generateAIVoice() {
    ...
    showLoading();
    fetch("http://localhost:5000/generate-voice", {
      method: "POST",
      ...
      .then((data) => {
        hideLoading();
        if (data.audio_url) {
          const audioUrl = `http://localhost:5000${data.audio_url}`; // 确保这里使用完整的 URL
      ...
      .catch((error) => {
        ...
        hideLoading();
        alert("生成AI配音时出错：" + (error.error || error.message));
      });
  }
  ...
  function initializeArticlesList() {
    const articles = [
      {
        name: "生活不是完美的",
        url: "https://gist.githubusercontent.com/tangruan/43ef1a512c296afe022233662d9b5f7a/raw/5eef715e78926bd8d8d35a54f90eed532ba363b9/Life%2520Is%2520Not%2520%2520Perfect",
      },
      {
        name: "时间流逝匆匆",
        url: "https://gist.githubusercontent.com/tangruan/18cd4471dba701dcc8babab16ee99a70/raw/1e6efa996797f00d9392395779d9ba330a08ce47/Time%2520doesn't%2520wait",
      },
      {
        name: "遵循你的内心",
        url: "https://gist.githubusercontent.com/tangruan/e88f274684748c740400867cd64d6dbf/raw/66fc88e36d4ff63b5e211660c262864191e9fa21/Always%2520Follow%2520Your%2520Heart",
      },
    ];
  ...
  function handleAudioProcessResult(result) {
    console.log('音频处理结果:', result);
    // 这里可以根据处理结果更新UI或进行其他操作
  }
